#!/usr/bin/env python
# 
# {wdi'notes}Modified class svm_model
# {wdi'notes}Added multiple functions to return w_2, rho, nSV, probA, and probB
...
if find_library('svm'):
	...
	if sys.platform == 'win32':
		libsvm = CDLL('E:/!WeiDI_Research/Reasearch_Program/wdi_PhD/libsvm-3.0.wdi/python/libsvm.dll')
	else:
...
class svm_model(Structure):
	...
	def get_w2_vector(self):
                """
                Added function to return |w|^2 value (Di,Wei)
                """
		nr_class = self.get_nr_class()
		nrs = nr_class *(nr_class-1)/2
		w2_vector = (c_double * nrs)()
		libsvm.svm_get_model_w2_vector(self,w2_vector)	
		return w2_vector[:nrs]
	...
	def get_nSV(self):
		...
		nr_class = self.get_nr_class()
		nSV = (c_int * nr_class)()
		libsvm.svm_get_model_nSV_vector(self, nSV)
		return nSV[:nr_class]

	def get_rho_vector(self):	# modified by wdi April-3 2010
                """
                Added function to return rho(Di,Wei)
                """
		nr_class = self.get_nr_class()
		nrs = nr_class *(nr_class-1)/2
		rho_vector = (c_double * nrs)()
		libsvm.svm_get_model_rho_vector(self,rho_vector)	
		return rho_vector[:nrs]


	def get_probA_vector(self):
                """
                Added function to return probA(Di,Wei)
                """
		nr_class = self.get_nr_class()
		nrs = nr_class *(nr_class-1)/2
		probA_vector = (c_double * nrs)()
		libsvm.svm_get_model_probA_vector(self,probA_vector)	
		return probA_vector[:nrs]

	def get_probB_vector(self):
                """
                Added function to return probB(Di,Wei)
                """
		nr_class = self.get_nr_class()
		nrs = nr_class *(nr_class-1)/2
		probB_vector = (c_double * nrs)()
		libsvm.svm_get_model_probB_vector(self,probB_vector)	
		return probB_vector[:nrs]
...
# Added functions to return w_2, rho, nSV, probA and probB (Di, Wei)#
