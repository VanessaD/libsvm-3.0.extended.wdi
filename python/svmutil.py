...
#################################################
####### Below is modified by Wei-di  #############
def distance_to_hyperplane(pred_values, svm_m, signed=False):
        '''
        ...
        '''
        w2 = svm_m.get_w2_vector()
        nr = svm_m.get_nr_class()
        dis2hyper = list()
        if not signed:
                for pv in pred_values:
                        dis2hyper.append([abs(v)/w for (v, w) in zip(pv, w2)])
        else:
                for pv in pred_values:
                        dis2hyper.append([v/w for (v, w) in zip(pv, w2)])
        return dis2hyper
